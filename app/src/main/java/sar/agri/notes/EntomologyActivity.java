...
public class EntomologyActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> ento = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.entomology);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				questi.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(length); _repeat20++) {
								if (ento.get((int)number).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || ento.get((int)number).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									ento.remove((int)(number));
								}
								number--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(ento));
	...
	public void onBackPressed() {
		questi.removeEventListener(_questi_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.ento_main, null);
			}
			...
			textview4.setText(ento.get((int)_position).get("Question").toString());
			textview5.setText(ento.get((int)_position).get("Answer").toString());
