...
public class SeedActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> Seed_Production = new ArrayList<>();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				seed_production.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						Seed_Production = new ArrayList<>();
						try {
							GenericTypeIndicator<HashMap<String, Object>> _ind = new GenericTypeIndicator<HashMap<String, Object>>() {};
							for (DataSnapshot _data : _dataSnapshot.getChildren()) {
								HashMap<String, Object> _map = _data.getValue(_ind);
								Seed_Production.add(_map);
							}
						}
						catch (Exception _e) {
							_e.printStackTrace();
						...
						if (_charSeq.length() > 0) {
							Num = Seed_Production.size() - 1;
							Len = Seed_Production.size();
							for(int _repeat20 = 0; _repeat20 < (int)(Len); _repeat20++) {
								if (Seed_Production.get((int)Num).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || Seed_Production.get((int)Num).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									Seed_Production.remove((int)(Num));
								}
								Num--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(Seed_Production));
	...
	public void onBackPressed() {
		seed_production.removeEventListener(_seed_production_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.seed_main, null);
			}
			...
			textview3.setText(Seed_Production.get((int)_position).get("Question").toString());
			textview4.setText(Seed_Production.get((int)_position).get("Answer").toString());
