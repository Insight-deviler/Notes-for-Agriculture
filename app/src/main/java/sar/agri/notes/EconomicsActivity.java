...
public class EconomicsActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> aec = new ArrayList<>();
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(length); _repeat20++) {
								if (aec.get((int)number).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || aec.get((int)number).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									aec.remove((int)(number));
								}
								number--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(aec));
	...
	public void onBackPressed() {
		que.removeEventListener(_que_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.eco_main, null);
			}
			...
			textview4.setText(aec.get((int)_position).get("Question").toString());
			textview5.setText(aec.get((int)_position).get("Answer").toString());
