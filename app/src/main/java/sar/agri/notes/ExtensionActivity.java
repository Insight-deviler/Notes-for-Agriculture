...
public class ExtensionActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> aex = new ArrayList<>();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				quest.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(length); _repeat20++) {
								if (aex.get((int)number).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || aex.get((int)number).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									aex.remove((int)(number));
								}
								number--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(aex));
	...
	public void onBackPressed() {
		quest.removeEventListener(_quest_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.aex_main, null);
			}
			...
			textview4.setText(aex.get((int)_position).get("Question").toString());
			textview5.setText(aex.get((int)_position).get("Answer").toString());
