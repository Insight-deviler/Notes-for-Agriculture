...
public class GpbActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> gpb = new ArrayList<>();
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(len); _repeat20++) {
								if (gpb.get((int)num).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || gpb.get((int)num).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
						...
						listview1.setAdapter(new Listview1Adapter(gpb));
	...
	public void onBackPressed() {
		question.removeEventListener(_question_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.gpb_main, null);
			}
			...
			textview3.setText(gpb.get((int)_position).get("Question").toString());
			textview4.setText(gpb.get((int)_position).get("Answer").toString());
