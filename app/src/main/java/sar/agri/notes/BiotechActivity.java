...
public class BiotechActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> Biotech = new ArrayList<>();
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				Biotechnology.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(l); _repeat20++) {
								if (Biotech.get((int)n).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || Biotech.get((int)n).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									Biotech.remove((int)(n));
								}
								l--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(Biotech));
	...
	public void onBackPressed() {
		Biotechnology.removeEventListener(_Biotechnology_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.biotech_main, null);
			}
			...
			textview3.setText(Biotech.get((int)_position).get("Question").toString());
			textview4.setText(Biotech.get((int)_position).get("Answer").toString());
