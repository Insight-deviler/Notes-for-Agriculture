...
public class MicrobiologyActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> AGM = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.microbiology);
					...
					public void onDataChange(DataSnapshot _dataSnapshot) {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(len); _repeat20++) {
								if (AGM.get((int)num).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || AGM.get((int)num).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
						...
						listview1.setAdapter(new Listview1Adapter(AGM));
	...
	public void onBackPressed() {
		questions.removeEventListener(_questions_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.micro_main, null);
			}
			...
			textview3.setText(AGM.get((int)_position).get("Question").toString());
			textview4.setText(AGM.get((int)_position).get("Answer").toString());
