...
public class GeneticsActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> gen_tics = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.genetics);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				Gene_tics.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						gen_tics = new ArrayList<>();
						try {
						...
						if (_charSeq.length() > 0) {
							...
							for(int _repeat20 = 0; _repeat20 < (int)(len); _repeat20++) {
								if (gen_tics.get((int)num).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || gen_tics.get((int)num).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									gen_tics.remove((int)(num));
								}
								num--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(gen_tics));
	...
	public void onBackPressed() {
		Gene_tics.removeEventListener(_Gene_tics_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.gen_tics, null);
			}
			...
			textview3.setText(gen_tics.get((int)_position).get("Question").toString());
			textview4.setText(gen_tics.get((int)_position).get("Answer").toString());
