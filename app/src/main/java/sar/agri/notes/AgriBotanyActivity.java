...
public class AgriBotanyActivity extends AppCompatActivity {
	...
	private ArrayList<HashMap<String, Object>> botany = new ArrayList<>();
	...
	protected void onCreate(Bundle _savedInstanceState) {
		...
		setContentView(R.layout.agri_botany);
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				Botany.addListenerForSingleValueEvent(new ValueEventListener() {
					@Override
					public void onDataChange(DataSnapshot _dataSnapshot) {
						botany = new ArrayList<>();
						try {
						...
						if (_charSeq.length() > 0) {
							number = botany.size() - 1;
							length = botany.size();
							for(int _repeat20 = 0; _repeat20 < (int)(length); _repeat20++) {
								if (botany.get((int)number).get("Question").toString().toLowerCase().contains(_charSeq.toLowerCase()) || botany.get((int)number).get("Answer").toString().toLowerCase().contains(_charSeq.toLowerCase())) {
									
								}
								else {
									botany.remove((int)(number));
								}
								number--;
							}
						}
						listview1.setAdapter(new Listview1Adapter(botany));
	...
	public void onBackPressed() {
		Botany.removeEventListener(_Botany_child_listener);
		finish();
	}
	...
	public class Listview1Adapter extends BaseAdapter {
		...
		public View getView(final int _position, View _view, ViewGroup _viewGroup) {
			...
			View _v = _view;
			if (_v == null) {
				_v = _inflater.inflate(R.layout.agri_bot, null);
			}
			...
			textview3.setText(botany.get((int)_position).get("Question").toString());
			textview4.setText(botany.get((int)_position).get("Answer").toString());
